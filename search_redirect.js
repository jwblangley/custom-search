...
const longestMatch = function(validKeys, key, delimeter) {
    ...
    while (i > 0) {
        ...
        if (subCommand in validKeys) {
            const args = key.substring(i + 1).startsWith(delimeter) ? key.substring(i + 1 + delimeter.length) : key.substring(i + 1)
...
const getRedirect = function (sanitisedSearchString) {
    ...
    if (!lm) {
        return fallback.replace(placeholder, sanitisedSearchString.replaceAll(SPACE, ENCODED_SPACE))
    }

    const [command, args] = lm
    return config[command].replace(placeholder, args.replaceAll(SPACE, ENCODED_SPACE))
}
...
window.onload = function () {
    ...
    const sanitisedSearchString = searchString.replace("?q=", "").replaceAll(ENCODED_SPACE, SPACE).trim()
    debugTable.appendChild(createTableRow("Sanitised search string", sanitisedSearchString))
